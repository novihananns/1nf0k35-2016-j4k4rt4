...
function delete_entry1($kode){
    ...
    function get_mail_config(){
     ...
     function get_pus($code=""){
        $query = ("SELECT cl_phc.value
                FROM  app_users_list
                INNER JOIN cl_phc
                ON SUBSTR(cl_phc.code,2)=app_users_list.code where app_users_list.code='$code' ");
    ...
    function update_profile($code){
        ...
        if($this->db->update('app_users_profile',$data,array('code' => $code))){
            return true;
        }else{
            return mysql_error();
        }
  ...
  function check_password($username,$code){
