...
<form action="<?php echo base_url()?>mst/model_kabupaten/dodel_multi" method="POST" name="">
  <div class="row">
    <!-- left column -->
    <div class="col-md-12">
        ...
        <div class="box-header">
	      ...
	      <div class="box-footer">
	      	<div class="row">
	      		...
	      		?>
				 	<button type="button" class="btn btn-primary" onclick="document.location.href='<?php echo base_url()?>mst/kabupaten/add'"><i class='fa fa-plus-square-o'></i> &nbsp; Tambah</button>
				<?php
						}
				?>
				...
				<div class="col-md-3">
				 ...
				 <div class="col-md-3">
				 	...
				 	<select name="provinsi" id="provinsi" class="form-control">
						...
						<?php foreach ($dataprovinsi as $pro ) { ;?>
						<?php $select = $pro->code == $this->session->userdata('filter_code_provinsi')  ? 'selected=selected' : '' ?>
							<option value="<?php echo $pro->code; ?>" <?php echo $select ?>><?php echo $pro->value; ?></option>
						<?php	} ;?>
			     	</select>
				 </div>
		 	</div>
...
$(function () {	
	   ...
	   var source = {
		...
		$("#jqxgrid").jqxGrid({		
			...
			columns: [
				...
				{ text: 'Kabupaten / Kota', datafield: 'value', columntype: 'textbox', align:'center', cellsalign:' center', filtertype: 'textbox', width: '50%' }
			]
		});

	function edit(code){
		document.location.href="<?php echo base_url().'mst/kabupatenkota/edit';?>/" + code;
	}
	
	function del(code){
		var confirms = confirm("Hapus Data ?");
		if(confirms == true){
			$.post("<?php echo base_url().'mst/kabupatenkota/dodel' ?>/" + code,  function(){
	...
	$('#provinsi').change(function(){
     ...
     // var id_mst_inv_ruangan = '<?php echo set_value('ruangan')?>';
      $.ajax({
        url : '<?php echo site_url('mst/Kabupatenkota/get_provinsifilter') ?>',
        type : 'POST',
    ...
    $("#btn-export").click(function(){
		...
		$.post("<?php echo base_url()?>mst/kabupaten/datakepalakeluaraexport",post,function(response	){
			//alert(response);
