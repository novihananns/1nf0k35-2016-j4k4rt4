  ...
  <div class="row">
    ...
    <div class="col-md-12">
      ...
      <div class="box box-primary">
	      ...
	      <div class="box-footer">
			...
			<div class="row">
				<div class="col-md-3">
				 ...
				 <div class="col-md-3" style="position:relative; left:250px;">
        ...
        <div class="box-body">
        <?php
			//	echo $this->session->userdata("filter_code_provinsi")." || prov";
			//	echo $this->session->userdata("filter_code_kota");
			?>
		    <div class="div-grid">
		        <div id="jqxgrid"></div>
			</div>
...
$(function () {	
		...
		$("#jqxgrid").jqxGrid({		
			...
			columns: [
			  { text: 'Cek', align: 'center', filtertype: 'none', sortable: false, width: '10%', cellsrenderer: function (row) {
                ...
                { text: 'Code', datafield: 'code', columntype: 'textbox', filtertype: 'textbox', width: '35%' },
                { text: 'Nama / Asal', datafield: 'nama', columntype: 'textbox', align:'left', filtertype: 'textbox',width: '35%' }

			]
		});

	function edit(code){
		document.location.href="<?php echo base_url().'mst/kecamatan/edit';?>/" + code;
	}
	function hapus(code){
			var confirms = confirm("Hapus Data ?");
		if(confirms == true){
			$.post("<?php echo base_url().'mst/kecamatan/dodel' ?>/" + code,  function(){
	...
	$('#provinsi').change(function(){
      ...
      $.ajax({
        url : '<?php echo site_url('mst/kecamatan/get_provinsifilter') ?>',
        type : 'POST',
    ...
    $('#kota').change(function(){
      var kota = $(this).val();
      $.ajax({
        url : '<?php echo site_url('mst/kecamatan/get_kotafilter') ?>',
        type : 'POST',
        data : 'kota=' + kota,
        success : function(data) {
          $('#kelurahan').html(data);
    ...
    $("#btn-export").click(function(){
		...
		post = post+'&provinsi='+$("#provinsi option:selected").text()+'&kota='+$("#kota option:selected").text()+'&kelurahan='+$("#kelurahan option:selected").text();
		
		$.post("<?php echo base_url()?>mst/kecamatan/data_export",post,function(response){
			//alert(response);
