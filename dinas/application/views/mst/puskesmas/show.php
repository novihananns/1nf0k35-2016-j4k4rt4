...
$(function () {	
	  ...
	  var source = {
			...
			{ name: 'cd_kompemilikbarang', type: 'number'},
			{ name: 'cd_propinsi', type: 'number'},			
			{ name: 'cd_kabkota', type: 'number'},
			{ name: 'cd_bidang', type: 'number'},
			{ name: 'cd_unitbidang', type: 'number'},
			{ name: 'cd_satuankerja', type: 'number'},
			{ name: 'alamat', type: 'string'},
			{ name: 'tlp', type: 'number'},
		...
		$("#jqxgrid").jqxGrid({		
			...
			columns: [
			  { text: 'Cek', align: 'center', filtertype: 'none', sortable: false, width: '5%', cellsrenderer: function (row) {
                	...
                	{ text: 'Hapus', align: 'center', filtertype: 'none', sortable: false, width: '5%', cellsrenderer: function (row) {
				    var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', row);
                ...
                { text: 'Nama', datafield: 'value', columntype: 'textbox', align:'left', filtertype: 'textbox',width: '20%' },
				{ text: 'Alamat', datafield: 'alamat', columntype: 'textbox', filtertype: 'textbox', width: '32%' },
				{ text: 'No Tlp', datafield: 'tlp', columntype: 'textbox', filtertype: 'textbox', width: '15%' }

			]
		});

	function edit(code){
		document.location.href="<?php echo base_url().'mst/puskesmas/edit';?>/" + code;
	}
	function hapus(code){
		var confirms = confirm("Hapus Data ?");
		if(confirms == true){
			$.post("<?php echo base_url().'mst/puskesmas/dodel' ?>/" + code,  function(){
	...
	$('#provinsi').change(function(){
      ...
      $.ajax({
        url : '<?php echo site_url('mst/puskesmas/get_provinsifilter') ?>',
        type : 'POST',
    ...
    $("#btn-export").click(function(){
		...
		post = post+'&provinsi='+$("#provinsi option:selected").text()+'&kota='+$("#kota option:selected").text()+'&kecamatan='+$("#kecamatan option:selected").text();
		
		$.post("<?php echo base_url()?>mst/puskesmas/data_export",post,function(response){
			//alert(response);
